...
| redis | ۲۰۰M | ۱۰۰M |
| **جمع فعلی** | **۱۷۶۴M (~1.75GB)** | — |
| حاشیهٔ باقی‌مانده برای فاز ۲ (EMQX ~۲۵۶M و…) | ~۱.۴۵ گیگ | — |
...
## ۸.۸ نکتهٔ آینده — Edge Agent (فاز ۲)
وقتی اتصال زندهٔ صنعتی فعال شد، یک **Edge Agent سبک** (کانتینر جدا، ایده‌آل روی یک دستگاه در همان شبکهٔ DMZ سایت مشتری، نه روی این سرور Hetzner) داده را از OPC-UA/Modbus محلی می‌خواند و فقط outbound (HTTPS) به این سرور push می‌کند. این سرور (`petro`) هرگز مستقیماً روی شبکهٔ OT حاضر نیست.
